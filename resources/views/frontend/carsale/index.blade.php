            ...
            @foreach($tenantcarousel as $key => $carousel)
                <div class="carousel-item {{ $key == 0 ? 'active' : '' }}">
                    <div class="carousel-background" 
                        style="background-image: url('{{ isset($carousel->image) ? route('file', $carousel->image) : asset('images/producto-sin-imagen.PNG') }}'); background-size: cover; background-position: center; height: 1200px;">
                    </div>
                    ...
                    <div class="row no-gutters">
                        <div class="col-md-4 d-flex align-items-center">
                            <form class="request-form ftco-animate bg-primary" action="{{ url('send-email/blog') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="telephone" required placeholder="Número de teléfono">
                                </div>
                                <div class="form-group">
                                    <label for="" class="label">E-mail</label>
                                    <input type="text" class="form-control" name="email" required placeholder="Correo electrónico">
                                </div>
                                <div class="form-group">
                                    <label for="" class="label">Consulta</label>
                                    <input type="text" class="form-control" name="question" required placeholder="¿Cuál es tu duda?">
                                </div>
                                <div class="form-group">
                        ...
                        <div class="col-md-8 d-flex align-items-center">
                            <div class="services-wrap rounded-right w-100">
                                ...
                                <p><a href="{{ url('compare/vehicles') }}" class="btn btn-primary py-3 px-4">Comparar vehículos</a></p>
                            </div>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="item">
                                    <div class="car-wrap rounded ftco-animate">
                                        ...
                                        <div class="text">
                                            ...
                                            <p class="d-flex mb-0 d-block"><a href="{{ url('detail-clothing/' . $item->id . '/' . $item->category_id) }}" class="btn btn-secondary py-2 ml-1">Detallar</a></p>
                                        </div>
                                    </div>
                                </div>
                            @endforeach
                ...
                <div class="row no-gutters">
                    ...
                    <div class="col-md-6 wrap-about ftco-animate">
                        <div class="heading-section heading-section-white pl-md-5">
                            ...
                            <p><a href="{{ url('https://wa.me/506' . $tenantinfo->whatsapp) }}" class="btn btn-primary py-3 px-4">Agendar cita</a></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="container">
            <div class="row justify-content-center mb-5">
                <div class="col-md-7 heading-section text-center ftco-animate">
                    <span class="subheading">Blog de {{ isset($tenantinfo->title) ? $tenantinfo->title : '' }}, explora nuestras secciones, y aclara las dudas acerca de nuestros servicios.</span>
                </div>
            </div>
            {{-- Condición para centrado si hay dos o menos blogs --}}
            <div class="row d-flex blog-container {{ count($blogs) <= 2 ? 'two-or-less' : '' }}">
                @foreach ($blogs as $item)
                    <div class="col-md-4 d-flex ftco-animate">
                        <div class="blog-entry justify-content-end">
                            <a href="blog-single.html" class="block-20" style="background-image: url('{{ isset($item->image) ? route('file', $item->image) : url('images/producto-sin-imagen.PNG') }}');"></a>
                            <div class="text pt-4">
                                <div class="meta mb-3">
                                    <div><a href="#">{{ $item->fecha_post }}</a></div>
                                    <div><a href="#">{{ $item->autor }}</a></div>
                                </div>
                                <h3 class="heading mt-2"><a href="{{ url('/blog/' . $item->id . '/' . $item->name_url) }}">{{ $item->title }}</a></h3>
                                <p><a href="{{ url('/blog/' . $item->id . '/' . $item->name_url) }}" class="btn btn-primary">Leer más</a></p>
                            </div>
                        </div>
                    </div>
                @endforeach
