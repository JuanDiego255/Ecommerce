        ...
        <div class="modal-content" style="border-radius:14px;border:none;">
            <div class="modal-header border-0 pb-0">
                <div>
                    ...
                    <p class="mb-0" style="font-size:.75rem;color:var(--gray3);">{{ $tenant->id }}</p>
                </div>
            ...
            <form action="{{ url('tenant-payment/store') }}" method="POST">
                ...
                <div class="modal-body" style="display:grid;gap:12px;">
                    <div>
                        ...
                        <input type="number" name="payment" class="filter-input"
...
<div class="page-header">
    <div>
        <p class="page-header-title">{{ $tenant->id }}</p>
        <p class="page-header-sub">Plan: {{ $tenant->plan }} · Historial de pagos</p>
    </div>
    <div class="d-flex gap-2">
        ...
        <a href="{{ url('tenants/payments') }}" class="act-btn ab-neutral" title="Volver">
        ...
        <div>
            <div class="tp-kpi-value">
                {{ \Carbon\Carbon::parse($lastPayment->payment_date)
                    ->addMonths(max(1, (int)($tenant->time_to_pay ?? 1)))
                    ->format('d/m/Y') }}
            </div>
            <div class="tp-kpi-label">Próx. cobro estimado</div>
        </div>
    </div>
    @endif
</div>
            ...
            <table class="table table-hover align-middle mb-0" id="tenants-pay">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end fw-semibold">{{ $fmt($p->payment) }}</td>
                        <td class="text-center">
                            <form method="POST" action="{{ url('/delete/pay/' . $p->id) }}"
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="text-center text-muted py-4">Sin pagos registrados</td>
                    </tr>
                    @endforelse
                </tbody>
                @if($payments->count() > 0)
                <tfoot>
                    <tr style="border-top:2px solid var(--gray1);">
                        <td class="fw-bold" style="font-size:.82rem;">Total</td>
...
<script>
var dtPay = $('#tenants-pay').DataTable({
